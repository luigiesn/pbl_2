...
void Protocol_Parse(ProtocolPacket *pkt, unsigned char *sentence){
    ...
    if(sentence[0] == PROTOCOL_START_BYTE && sentence[PROTOCOL_LENGTH - 1] == PROTOCOL_END_BYTE){ // check START and END bytes are valid
        ParamType pTi;

        for(pTi = 0 ; pTi < prtEND ; pTi++){ // check command type
            char *cPtr = cmds[pTi];

            if(sentence[1] == cPtr[0] && sentence[2] == cPtr[1]){
                pkt->prt = pTi;
                pkt->value = (sentence[3] - '0')*100 + (sentence[4] - '0')*10 + (sentence[5] - '0'); // parse from character to integer
            }
        }
    }
